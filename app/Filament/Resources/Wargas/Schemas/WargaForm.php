            ...
            ->components(
                [
                    Section::make('Wilayah RT/RW')
                        ->description('Tentukan wilayah tempat warga ini terdaftar.')
                        ->icon('heroicon-o-home-modern')
                        ->columnSpanFull()
                        ->schema([

                            Placeholder::make('tenant_kosong')
                                ->label('')
                                ->content('⚠️ Akun Anda belum terhubung ke wilayah RT/RW (Tenant). Hubungi Administrator.')
                                ->visible(
                                    fn() =>
                            ...
                            Select::make('tenant_id')
                                ...
                                ->native(false)
                                ->required()
                                ->placeholder('Cari wilayah RT/RW aktif...')
                                ->default(fn() => Auth::user()?->isAdmin() ? null : Auth::user()?->tenant_id)
                                ->disabled(fn() => ! Auth::user()?->isAdmin())
                                ->dehydrated(true)
                                ->helperText(
                                    fn() => Auth::user()?->isAdmin()
                                        ? 'Ketik nama wilayah RT/RW aktif terlebih dahulu.'
                                        : 'Wilayah otomatis mengikuti akun Anda.'
                                ),
                        ])
                        ->disabled(
                            fn() =>
